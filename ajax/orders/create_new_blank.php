...
foreach ($arBufer as $order) {
    if ($order['PROPERTY_ARRIVAL_DATE_VALUE'] == $arProperties[11][0]["VALUE"] &&
        ...
        $arBlankFields['ORDER_ID'] = $order['ID'];
...
$arSelect = array("ID", "NAME", "IBLOCK_SECTION_ID", "PROPERTY_OBJECT_COST", "PROPERTY_COST_PER_PERSON", "PROPERTY_CAPACITY_MAXIMUM", "PROPERTY_CAPACITY_ESTIMATED", "PROPERTY_CAR_POSSIBILITY", "PROPERTY_LOCATION", "PROPERTY_FIXED_COST");
$arFilter = array("IBLOCK_ID" => IB_LOCATIONS, "ID" => $arProperties[21][0]['VALUE']);
$res = CIBlockElement::GetList(array(), $arFilter, false, array(), $arSelect);
if ($ar_res = $res->GetNextElement()) {
    $arFields = $ar_res->GetFields();
    $arBlankFields['OBJECT_NAME'] = $arFields['NAME'];
    $arBufer['OBJECT_ID'] = $arFields['ID'];

    if(!$arBlankFields['OBJECT_COST']){
        $arBlankFields['OBJECT_COST'] = $arFields['PROPERTY_FIXED_COST_VALUE'];
    }
    ...
    $arBufer['CAR_POSSIBILITY'] = $arFields['PROPERTY_CAR_POSSIBILITY'];
}

$arObjectSections = getObjectSections($arBufer['OBJECT_ID']);
if ($arObjectSections) {
    $arBlankFields['OBJECT_CATEGORY'] = $arObjectSections[1]['NAME'];
    $arBlankFields['OBJECT_LOCATION'] = $arObjectSections[2]['NAME'];
}
...
<div class="blank_right">
    ...
    <div class="i-blank-description">
        ...
        зоне!
        Пребывая на территории , при себе
