<?
if (!defined("B_PROLOG_INCLUDED") || B_PROLOG_INCLUDED !== true) die();
/** @var array $arParams */
/** @var array $arResult */
/** @global CMain $APPLICATION */
/** @global CUser $USER */
/** @global CDatabase $DB */
/** @var CBitrixComponentTemplate $this */
/** @var string $templateName */
/** @var string $templateFile */
/** @var string $templateFolder */
/** @var string $componentPath */

/** @var CBitrixComponent $component */

use Bitrix\Main\Application,
    Bitrix\Main\Context,
    Bitrix\Main\Request,
    Bitrix\Main\Server;

$this->setFrameMode(true);

$APPLICATION->SetTitle("Заказы");
$context = Application::getInstance()->getContext();
$request = $context->getRequest();

$filter_code = $request->getQuery("code");
$filter_date = $request->getQuery("date");
$filter_orders = $request->getQuery("orders");
$filter_locations = $request->getQuery("locations");
$isArch = $request->getQuery("arch");
$search_fio = $request->getQuery("search_fio");
$search_car_id = $request->getQuery("search_car_id");
$search_object_name = $request->getQuery("search_object_name");

$user = getUserData();
if (isset($user['USER_LOCATIONS']) && !empty($user['USER_LOCATIONS'])) {
    foreach ($user['USER_LOCATIONS'] as $user_locations) {
        $arUserLocations[] = $user_locations['ID'];
    }
}
if (isset($user['USER_OBJECTS']) && !empty($user['USER_OBJECTS'])) {
    foreach ($user['USER_OBJECTS'] as $user_object) {
        $arUserObjects[] = $user_object['ID'];
    }
}

if ((isset($filter_orders) && !empty($filter_orders)) || (isset($filter_locations) && !empty($filter_locations))) {
    $btn_name = 'Все заказы';
    $btn_action = 'all';
} else {
    $btn_name = 'Мои заказы';
    $btn_action = 'my';
}

?>
<? if ($arResult["MESSAGE"] <> ''): ?>
    <? ShowNote($arResult["MESSAGE"]) ?>
<? endif ?>
<form action="" class="orders-filter">
    <div class="orders-filter_wrap">
        <div class="input input--flex">
            <label for="" class="input-label">Уникальный код</label>
            <input type="text" value="<?= $filter_code ? $filter_code : '' ?>" id="order-id-filter">
        </div>
        <div class="input input--flex">
            <label for="" class="input-label">Дата</label>
            <input type="text" value="<?= $filter_date ? $filter_date : '' ?>" class="input-date js-input-date"
                   id="order-date-filter">
        </div>
    </div>
    <div class="orders-filter_wrap">
        <input type="button"
               value="Применить"
               class="gray-btn"
               id="set-filter-btn"
        >
        <? if (count($user['USER_LOCATIONS']) > 1): ?>
            <div class="select-block" id="user-location-filter">
                <div class="custom-select">
                    <div class="custom-select_head">
                                    <span class="custom-select_title"
                                          data-selected-id="">Выберите локацию</span>
                        <svg class="custom-select_icon" width="14" height="8" viewBox="0 0 14 8"
                             fill="none"
                             xmlns="http://www.w3.org/2000/svg">
                            <path d="M1 1L7 7L13 1" stroke="#000"/>
                        </svg>
                    </div>
                    <div class="custom-select_body">
                        <? foreach ($user['USER_LOCATIONS'] as $userLocation): ?>
                            <div class="custom-select_item"
                                 data-id="<?= $userLocation['ID'] ?>"><?= $userLocation['NAME'] ?>
                            </div>
                        <? endforeach; ?>
                        <div class="custom-select_item"
                             data-id="">Все локации
                        </div>
                    </div>
                </div>
            </div>
            <? /* else: */ ?><!--
            <input type="button"
                   value="<? /*= $btn_name */ ?>"
                   class="gray-btn"
                   id="my-orders-filter-btn"
                   data-action="<? /*= $btn_action */ ?>"
                <? /*= isset($arUserObjects) ? 'data-obj-id="' . CUtil::PhpToJSObject($arUserObjects) . '"' : "" */ ?>
                <? /*= isset($arUserLocations) ? 'data-loc-id="' . CUtil::PhpToJSObject($arUserLocations) . '"' : "" */ ?>
            >-->
        <? endif; ?>
        <? if ($user['IS_ADMIN'] && in_array(7, $user['GROUPS'])): ?>
            <button class="gray-btn" id="orders-archive-btn">Архив заказов</button>
        <? endif; ?>
    </div>
</form>

<form class="orders-filter" id="search-form-block">
    <div class="orders-filter_wrap">
        <div class="input input--flex">
            <label for="search-field-fio" class="input-label">Фамилия</label>
            <input type="text" value="<?= $search_fio ? $search_fio : '' ?>" id="search-field-fio">
        </div>
        <div class="input input--flex">
            <label for="search-field-car-id" class="input-label">Номер машины</label>
            <input type="text" value="<?= $search_car_id ? $search_car_id : '' ?>" id="search-field-car-id">
        </div>
        <div class="input input--flex">
            <label for="search-field-car-id" class="input-label">Название объекта</label>
            <input type="text" value="<?= $search_object_name ? $search_object_name : '' ?>"
                   id="search-field-object-name">
        </div>
    </div>
    <div class="orders-filter_wrap">
        <input type="button"
               value="Поиск"
               class="gray-btn"
               id="search-btn"
        >
    </div>
</form>

<? if ($arResult["NO_USER"] == "N"): ?>
    <? if (!empty($arResult['ELEMENTS'])): ?>
        <div class="table-wrap">
            <table class="table">
                <tr>
                    <th>Код</th>
                    <th>Название объекта</th>
                    <th>Кто забронировал</th>
                    <th>Даты брони</th>
                    <th>Время заезда</th>
                    <th>Состав группы</th>
                    <th>Дата и время выезда</th>
                    <th>Транспорт</th>
                    <th></th>
                    <th></th>
                    <th></th>
                </tr>
                <? foreach ($arResult['ELEMENTS'] as $element): ?>
                    <? if (isset($element['RED']) && $element['RED'] == 'true' && $element['OBJECT']['RENT_STATUS'] === true): ?>
                        <tr class="tr-red">
                    <? else: ?>
                        <tr>
                    <? endif; ?>
                    <td><?= $element['ID'] ?></td>
                    <td><?= $element['OBJECT']['NAME'] ?><br><?= $element['OBJECT']['LOCATION'] ?></td>
                    <td><?= $element['USER_FIO'] ?>
                        <? if ($element['BOOKING_TYPE'] == 'Онлайн'): ?>
                            <div class="online"><?= $element['BOOKING_TYPE'] ?></div>
                        <? else: ?>
                            <div class="operator">
                                <span class="operator_title">оператор</span>
                                <span class="operator_value"><?= $element['BOOKING_TYPE'] ?></span>
                            </div>
                        <? endif; ?>
                    </td>
                    <td>с <?= $element['ARRIVAL_DATE'] ?><br>
                        по <?= $element['DEPARTURE_DATE'] ?>
                    </td>
                    <td><?= $element['CHECK_IN_TIME'] ?></td>
                    <td>
                        всего - <?= $element['GROUP']['ADULTS_COUNT'] ?> чел <br>
                        <? if ($element['GROUP']['BENIFICIARIES_COUNT'] > 0): ?>
                            льготников - <?= $element['GROUP']['BENIFICIARIES_COUNT'] ?> чел
                        <? endif; ?>
                    </td>
                    <td><?= $element['DEPARTURE_DATE'] ?> <br>
                        <?= $element['DEPARTURE_TIME'] ?>
                    </td>
                    <td>
                        <? if (isset($element['ORDER_TRANSPORT']) && !empty($element['ORDER_TRANSPORT'])): ?>
                            <? foreach ($element['ORDER_TRANSPORT'] as $transportId): ?>
                                <?= $transportId ?><br>
                            <? endforeach; ?>
                        <? endif; ?>
                    </td>
                    <? if (isset($element['OBJECT']['RENT_STATUS'])): ?>
                        <? if ($element['OBJECT']['RENT_STATUS'] === false): ?>
                            <td>
                                <span class="available-text">Освободилось</span>
                            </td>
                        <? else: ?>
                            <td>
                                <button class="btn-available js-open-r-modal" data-name="modal-cancel-reservation"
                                        data-object-id="<?= $element['OBJECT']['ID'] ?>"
                                        data-hlb-order-id="<?= $element['HLBLOCK_ORDER_ID'] ?>"
                                        data-object-arrival-date="<?= $element['ARRIVAL_DATE'] . ' ' . $element['CHECK_IN_TIME'] ?>"
                                        data-object-departure-date="<?= $element['DEPARTURE_DATE'] . ' ' . $element['DEPARTURE_TIME'] ?>"
                                        onclick="free(this)"
                                >
                                    <svg width="13" height="13" viewBox="0 0 13 13" fill="none"
                                         xmlns="http://www.w3.org/2000/svg">
                                        <path
                                                d="M1.44375 12.9967L0 11.5529L5.05313 6.4998L0 1.44668L1.44375 0.00292969L6.49688 5.05606L11.55 0.00292969L12.9938 1.44668L7.94063 6.4998L12.9938 11.5529L11.55 12.9967L6.49688 7.94356L1.44375 12.9967Z"
                                                fill="#ED8C00"/>
                                    </svg>
                                    <span>Освободить</span>
                                </button>
                            </td>
                        <? endif; ?>
                    <? else: ?>
                        <td>
                            <span class="available-text">Освободилось</span>
                        </td>
                    <? endif; ?>
                    <? if ($isArch != 'Y'): ?>
                        <td>
                            <a href="<?= $arParams["EDIT_URL"] ?>?edit=Y&amp;CODE=<?= $element["ID"] ?><?= $element["IS_FAST"] ? '&fast=Y' : '' ?>"
                               class="btn-edit">
                                <svg width="18" height="18" viewBox="0 0 18 18" fill="none"
                                     xmlns="http://www.w3.org/2000/svg">
                                    <path
                                            d="M1.5 17.9997V15.731H16.5V17.9997H1.5ZM3.0375 13.7435V11.2497L10.0312 4.25599L12.525 6.74974L5.53125 13.7435H3.0375ZM13.35 5.92474L10.8563 3.43099L12.4313 1.85599C12.5688 1.69349 12.725 1.60911 12.9 1.60286C13.075 1.59661 13.25 1.68099 13.425 1.85599L14.8875 3.31849C15.05 3.48099 15.1312 3.65286 15.1312 3.83411C15.1312 4.01536 15.0625 4.18724 14.925 4.34974L13.35 5.92474Z"
                                            fill="#ED8C00"/>
                                </svg>
                                <span>Редактировать</span>
                            </a>
                        </td>
                        <? if ($user['IS_ADMIN']): ?>
                            <td>
                                <a class="btn-remove"
                                   href="?delete=Y&amp;CODE=<?= $element["ID"] ?>&amp;<?= bitrix_sessid_get() ?>"
                                   onClick="return confirm('<? echo CUtil::JSEscape(str_replace("#ELEMENT_NAME#", $element["NAME"], GetMessage("IBLOCK_ADD_LIST_DELETE_CONFIRM"))) ?>')"
                                >
                                    <svg width="20" height="20" viewBox="0 0 20 20" fill="none"
                                         xmlns="http://www.w3.org/2000/svg">
                                        <path
                                                d="M5.79167 17.083C5.47222 17.083 5.19444 16.965 4.95833 16.7288C4.72222 16.4927 4.60417 16.208 4.60417 15.8747V4.54134H3.75V3.60384H7.3125V3.02051H12.6875V3.60384H16.25V4.54134H15.3958V15.8747C15.3958 16.208 15.2778 16.4927 15.0417 16.7288C14.8056 16.965 14.5278 17.083 14.2083 17.083H5.79167ZM7.9375 14.3747H8.89583V6.29134H7.9375V14.3747ZM11.1042 14.3747H12.0625V6.29134H11.1042V14.3747Z"
                                                fill="#ED8C00"/>
                                    </svg>
                                </a>
                            </td>
                        <? else: ?>
                            <td></td>
                        <? endif; ?>
                    <? endif; ?>
                    </tr>
                <? endforeach; ?>
            </table>
        </div>
        <? if ($arResult["NAV_STRING"] <> ''): ?>
            <?= $arResult["NAV_STRING"] ?>
        <? endif ?>
    <? else: ?>
        <p>Список пуст</p>
    <? endif; ?>
<? endif ?>

