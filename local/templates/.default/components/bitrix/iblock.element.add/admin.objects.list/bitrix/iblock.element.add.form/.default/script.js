function deletePic(element) {
    ...
    let file_id = $(element).data('deleteId');
    $.ajax({
        url: '/ajax/objects/delete_picture.php',
        method: 'post',
...
function setDropDown() {
    let select = $('.custom-select');
    $(select).each(function () {
        let selected_item = $(this).find('.custom-select_item[data-selected="true"]');
        if (selected_item) {
            let selected_id = selected_item.data('id');
            let selected_value = selected_item.html();
            let select_title = $(this).find('.custom-select_title');
            select_title.attr('data-selected-id', selected_id);
            select_title.html(selected_value);
        }
    });
}
...
$(window).on('load', function () {
    let img_counter = $('#img-counter').val();
    let preview_counter = $('#preview-counter').val();
    ...
    onTimeLimitSelect();
    onCarPossibilityChange('car_possibility_radio_btn_0');
});
