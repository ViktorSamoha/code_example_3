...
function updateForm(params) {
    if (params) {
        if (params.TIME_UNLIMIT && params.TIME_UNLIMIT === "Нет") {
            if (params.TIME_INTERVAL && params.TIME_INTERVAL !== "Сутки") {
                $('#booking-period').hide();
            }
        }
    ...
    if (checkForm(
        ...
        BX.ajax.runComponentAction(
            ...
            function (response) {
                if (response) {
                    if (response.status === 'success') {
                        let modal = $('.modal[data-name="modal-success-order-edit-notification"]');
