    ...
    <div class="table-wrap">
        <table class="table">
            ...
            <? foreach ($arResult['ELEMENTS'] as $element): ?>
                ...
                <? endif; ?>
                <td><?= $element['ID'] ?></td>
                <td><?= $element['OBJECT_NAME'] ?><br><?= $element['OBJECT_LOCATION'] ?></td>
                <td>
                ...
                <td>
                    всего - <?= $element['GUEST_COUNT'] ?> чел <br>
                    <? if ($element['BENEFICIARIES'] > 0): ?>
                        льготников - <?= $element['BENEFICIARIES'] ?> чел
                    <? endif; ?>
                </td>
                <td><?= $element['DEPARTURE_DATE'] . ' ' . $element['DEPARTURE_TIME'] ?></td>
                <td>
                    <? if ($element['TRANSPORT']): ?>
                        <? foreach ($element['TRANSPORT'] as $arTransportData): ?>
                            <?= $arTransportData['TYPE'] . ' ' . $arTransportData['MODEL'] ?><br>
                        <? endforeach; ?>
                    <? endif; ?>
                </td>
                <? if (!isset($arParams['FILTER_VALUE']['ARCHIVE']) || $arParams['FILTER_VALUE']['ARCHIVE'] == 'N'): ?>
                    <td>
                        <? if ($element['OBJECT_ID'] && $element['HLBLOCK_ORDER_ID'] && $element['ARRIVAL_DATE'] && $element['CHECK_IN_TIME'] && $element['DEPARTURE_DATE'] && $element['DEPARTURE_TIME']): ?>
                            <button class="btn-available" type="button"
                                    onclick="unsetObjectBooking(<?= $element['OBJECT_ID'] ?>, <?= $element['HLBLOCK_ORDER_ID'] ?>, '<?= $element['ARRIVAL_DATE'] . ' ' . $element['CHECK_IN_TIME'] ?>', '<?= $element['DEPARTURE_DATE'] . ' ' . $element['DEPARTURE_TIME'] ?>', <?= $element['ID'] ?>)">
                    ...
                    <td>
                        <a href="/admin/booking/?EDIT=Y&ORDER_ID=<?= $element['ID'] ?>"
                           class="btn-edit">
                ...
                <? endif; ?>
                <td>
                    <? if ($element['ID'] && $element['HLBLOCK_ORDER_ID']): ?>
                        <a class="btn-remove"
                           href="javascript:void(0);"
                           onclick="deleteOrder(<?= $element['ID'] ?>, <?= $element['HLBLOCK_ORDER_ID'] ?>, <?= $element['IS_ROUTE'] ?>)"
                        >
