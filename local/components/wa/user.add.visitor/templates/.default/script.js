...
function addVisitorCheckReqFields() {
    let form = $('#add-user-group-element-form');
    ...
    if (!$('#checkbox_personal-data-confirm').is(':checked')) {
        ar_errors.push("Необходимо дать согласие на обработку персональных данных!<br>");
    }
    if (!$('#checkbox_visiting-rules-confirm').is(':checked')) {
...
$(document).ready(function () {
    ...
    flatpickr($('input[name="U_PREF_DOC_DATE"]'), {
        dateFormat: "d.m.Y",
        allowInput: "true",
    ...
    //submit form logic
    $('#add-user-group-element').click(function (e) {
        e.preventDefault();
        resetErrors();
        if (addVisitorCheckReqFields()) {
            let form = $('#add-user-group-element-form');
