...
//\Bitrix\Main\Diag\Debug::dumpToFile($_REQUEST, $varName = '$_REQUEST', $fileName = 'comp.txt');
...
if ($bAllowAccess) {
    ...
    if (check_bitrix_sessid() && (!empty($_REQUEST["iblock_submit"]) || !empty($_REQUEST["iblock_apply"]))) {
        ...
        //\Bitrix\Main\Diag\Debug::dumpToFile($arProperties, $varName = '$arProperties', $fileName = 'comp.txt');
        ...
        //\Bitrix\Main\Diag\Debug::dumpToFile($arResult["ERRORS"], $varName = '$arResult["ERRORS"]', $fileName = 'comp.txt');
        ...
        if (empty($arResult["ERRORS"])) {
            ...
            //\Bitrix\Main\Diag\Debug::dumpToFile($arUpdateValues, $varName = '$arUpdateValues', $fileName = 'comp.txt');
            //\Bitrix\Main\Diag\Debug::dumpToFile($arParams, $varName = '$arParams', $fileName = 'comp.txt');
            ...
            if ($arParams["ID"] > 0) {
                ...
                //\Bitrix\Main\Diag\Debug::dumpToFile($bFieldProps, $varName = '$bFieldProps', $fileName = 'comp.txt');

                $dbPropV = CIBlockElement::GetProperty($arParams["IBLOCK_ID"], $arParams["ID"], "sort", "asc", array("ACTIVE" => "Y"));
                while ($arPropV = $dbPropV->Fetch()) {

                    //\Bitrix\Main\Diag\Debug::dumpToFile($arPropV, $varName = '$arPropV[i]', $fileName = 'comp.txt');
